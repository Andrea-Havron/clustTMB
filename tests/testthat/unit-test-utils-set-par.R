...
test_that("test set.MuVarPow, expmod = TRUE", {
  ...
  mod <- MoE_clust(y, G=ng, gating= ~ 1, expert= ~ sex,
  ...
  for (g in 1:ng) {
    for (j in 1:ncol(y)) {
      ...
      Pars <- set.MuVarPow(Class. = Class[,g], 
                           ...
                           family. = gaussian() )
      expect_equal(unname(mod.sub$coefficients), 
                   Pars$mu_init)
      expect_equal(unname(var(mod.sub$residuals)),
                   Pars$var_init)
    }
  }
