...
## Random Effect Covariace Matrices
...
#### Spatial Example
Prior to fitting a spatial cluster model with clustTMB, users need to set up the constrained Delaunay Triangulation network using INLA. For guidance on setting up an appropriate mesh, see [Triangulation details and examples](https://becarioprecario.bitbucket.io/spde-gitbook/ch-intro.html#sec:mesh) and [Tools for mesh assessment](https://becarioprecario.bitbucket.io/spde-gitbook/ch-intro.html#sec:toolsmesh) from 
...
```{r run INLA meshbuilder, eval = FALSE}
library(clustTMB)
library(sp)
library(INLA)
data("meuse")
...
```{r meuse mesh}
Loc <- meuse[,1:2]
Bnd <- INLA::inla.nonconvex.hull(as.matrix(Loc), convex=200)
meuse.mesh <- INLA::inla.mesh.2d(as.matrix(Loc),
                           max.edge = c(300,1000),
                           boundary = Bnd)
```
...
```{r set up model}
 ...
 mod <- clustTMB(response = meuse[,3:6], 
