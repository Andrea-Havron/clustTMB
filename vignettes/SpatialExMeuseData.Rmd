...
The meuse data set from the sp library (Pebesma and Bivand, 2005) consists of 155 observations, 4 response variables, location data, and 8 covariates. The response involves four heavy metals measured to monitor pollution levels in the top soil of the Meuse river floodplain, Netherlands (Fig. \@ref(fig:sample), Rikken and van Rijn, 1993). The heavy metal data exhibit lognormality (Fig. \@ref(fig:pairs-expl)), allowing for a comparison between the Gaussian and lognormal distributions in addition to an FMM with and without spatial structure influencing the cluster probability.
...
ggm2 <- ggplot2::ggplot() +
  ...
  ggspatial::annotation_north_arrow(which_north = "grid", height = unit(1, "cm"), width = unit(1, "cm"),   
                         pad_y = unit(4, "cm"))
...
```{r}
mod.ln <- clustTMB( response = meuse[,3:6],
                    family = lognormal(link = "identity"),
                    G = 3, covariance.structure = "VVV" )
```

## Spatial FMM
The inclusion of spatial random effects in the expectation of the cluster probability in clustTMB depends on the SPDE-FEM approximation to a spatial Gaussian Markov Random Field (GMRF) introduced by the package, [R-INLA](https://www.r-inla.org/). See [Spatial GMRF] and [Gating Network] for details on these clustTMB formulations.

### Setting up the spatial mesh
The spatial model in clustTMB first requires the definition of the spatial mesh using INLA. This mesh defines the discretization of the continuous space using a constrained Delaunay triangulation. For details on building the spatial mesh, see [Krainski et al., 2021, Sec 2.6](https://becarioprecario.bitbucket.io/spde-gitbook/ch-intro.html#sec:mesh). See [instructions](https://www.r-inla.org/download-install) for downloading and installing R-INLA. The R-INLA package includes a user friendly [shiny app](https://becarioprecario.bitbucket.io/spde-gitbook/ch-intro.html#sec:toolsmesh) to aid in spatial mesh setting choices:
```{r, eval = FALSE}
#install R-INLA
install.packages("INLA",repos=c(getOption("repos"),INLA="https://inla.r-inla-download.org/R/stable"), dep=TRUE)
library(INLA)
meshbuilder() #launches INLA mesh shiny app
```

As an example, the spatial mesh for the meuse data is built using the R-INLA functions, inla.convex.hull() to set up the boundary and inla.mesh.2d() to generate the spatial mesh:
```{r}
library(INLA)
Loc <- meuse[,1:2]
Bnd <- inla.nonconvex.hull(as.matrix(Loc), convex=200)
meuse.mesh <- inla.mesh.2d(as.matrix(Loc),
                           max.edge = c(300,1000),
                           boundary = Bnd)
```
